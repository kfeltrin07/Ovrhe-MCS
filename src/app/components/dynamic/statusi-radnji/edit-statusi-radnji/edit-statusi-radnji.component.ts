...
import { CRUDAction, StatusiRadnji, Tvrtke } from 'src/app/models/models.service';
...
@Component({
  selector: 'app-edit-statusi-radnji',
  standalone: true,
  imports: [
    MatInputModule,
    MatButtonModule,
  ...
  templateUrl: './edit-statusi-radnji.component.html',
  styleUrl: './edit-statusi-radnji.component.scss'
})
export class EditStatusiRadnjiComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public receivedStatusiRadnji: StatusiRadnji,
    private dialogRef: MatDialogRef<EditStatusiRadnjiComponent>,
  ...
  public validateForm(statusiRadnji: StatusiRadnji): boolean {
    if (statusiRadnji.IDSTATUSARADNJE != '' && statusiRadnji.NAZIVSTATUSA != '') {
      return false;
    } else {
      return true;
    }
    ...
    this.http.post(
      this.globalVar.APIHost + this.globalVar.APIFile,
      {
        ...
        data: {
          ...
          pNazivStatusa: this.statusiRadnji.NAZIVSTATUSA,
          pJeZapoceto: this.statusiRadnji.JEZAPOCETO,
          pJeZavrseno: this.statusiRadnji.JEZAVRSENO,
          pSistemskiStatus: this.statusiRadnji.SISTEMSKISTATUS,
          pIdTvrtke: this.statusiRadnji.IDTVRTKE,
          pAktivnost: this.statusiRadnji.AKTIVNOST,
        }
      }
  ...
  public setTvrtkeFromDialog(Tvrtke?: Tvrtke): void {
    if (Tvrtke) {
      this.statusiRadnji.IDTVRTKE = Tvrtke.IDTVRTKE;
  ...
  public OfferedTvrtke(): void {
    this.http.post(
      this.globalVar.APIHost + this.globalVar.APIFile,
      {
        ...
        sid: this.session.loggedInUser.sessionID,
        data: {
          pDioNaziva: this.statusiRadnji.IDTVRTKE,
          limit: 100,
          page: 1,
          sort: [
            {
      ...
      const matchedItem = this.offeredTvrtke.find(item => item.IDTVRTKE.toUpperCase() === this.statusiRadnji.IDTVRTKE.toUpperCase());
      if (matchedItem) {
        this.statusiRadnji.IDTVRTKE = matchedItem.IDTVRTKE;
